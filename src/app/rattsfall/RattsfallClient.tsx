...
export default function RattsfallClient({ courtCases }: Props) {
  ...
  const featured3 = useMemo(
    () => [...courtCases].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 3),
    [courtCases]
  );
  const featuredIds = useMemo(
    () => new Set(featured3.map((courtCase) => courtCase.id)),
    [featured3]
  );
  const years = useMemo(
    () => Array.from(new Set(courtCases.map((courtCase) => courtCase.year))).sort((a, b) => b - a),
    [courtCases]
  );
  const topics = useMemo(
    () => ["Alla", "Vägledande", ...Array.from(new Set(courtCases.map((courtCase) => courtCase.topic))).sort()],
    [courtCases]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [courtCases, featuredIds, topic, year, search]);
