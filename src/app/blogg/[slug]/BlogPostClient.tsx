...
export default function BlogPostClient({ post, prevPost, nextPost }: Props) {
  const publicRelatedAgreements = post.relatedAgreements.filter(isVerifiedAgreement);
  return (
    <>
      {/* Hero */}
      ...
      <section className="py-6 sm:py-8">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <AnimatedSection>
            <div className="rounded-sm bg-[#E8EEE9] border-l-[3px] border-l-primary p-5">
              <p className="text-[16px] sm:text-[17px] text-text-primary leading-[1.65]">{post.aeoAnswer}</p>
            </div>
          </AnimatedSection>
        </div>
      </section>
          ...
          {post.content.map((section, i) => (
            <AnimatedSection key={i} delay={i * 0.05}>
              <div className="mb-8 sm:mb-10">
                ...
                {section.text.split("\n\n").map((paragraph, j) => (
                  <p key={j} className="text-text-primary leading-[1.7] mb-4 text-[16px] sm:text-[17px]">
                    {paragraph}
                  </p>
                ))}
              </div>
      ...
      {/* Related links */}
      {(publicRelatedAgreements.length > 0 || post.relatedOccupations.length > 0) && (
        <section className="py-8 sm:py-12">
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            {publicRelatedAgreements.length > 0 && (
              <AnimatedSection>
                <h3 className="font-bold text-text-primary text-sm mb-3">Relaterade avtal</h3>
                <div className="flex flex-wrap gap-2 mb-6">
                  {publicRelatedAgreements.map((slug) => (
                    <Link
                      key={slug}
                      href={`/avtal/${slug}`}
                      className="rounded-sm bg-card border border-border px-3 py-2 text-sm font-medium text-text-secondary hover:text-primary hover:border-primary transition-colors min-h-[40px]"
                    >
                      {slug}
                    </Link>
                  ))}
      ...
      <section className="py-6">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <a
            ...
            className="block rounded-sm p-8 sm:p-10 text-center border border-[#D8C7B1] bg-[#EFE7DA] hover:border-accent transition-colors"
          >
            ...
            <span className="inline-block mt-4 px-8 py-3 rounded-sm text-white font-semibold text-[16px]" style={{ background: "#B56A2D" }}>
