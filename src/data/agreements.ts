...
- Avsluta ALLTID med: "Kontakta Kommunal för bindande besked."`,
  ...
  ].map(a => ({ ...a, aiSystemPrompt: `Du är en AI-expert på ${a.name} — avtal för ~${a.employeeCount.toLocaleString('sv-SE')} anställda. ${a.wageTable[0] ? `Löner: ${['Individuell','Sifferlöst','Jämförbar nivå','Provisionsbaserad','Royaltybaserad','Individuell + provision','Individuell (mycket högt)','Sifferlöst (mycket högt)','Sifferlöst (högt)','Sifferlöst (extremt högt)','Individuell (högt)','Individuell + utlandstillägg','Projektbaserad','Beredskapsersättning','Individuell (per hopp)','Individuell (ca 50 000 kr)','Individuell (ca 45 000 kr)','Individuell (ca 35 000 kr)','Individuell (ca 32 000 kr)','Timlön 800–1200 kr/tim'].includes(a.wageTable[0].minimum) ? 'Individuell/variabel lönesättning' : `min ~${a.wageTable[0].minimum}`}${a.wageTable[0].median && a.wageTable[0].median !== 'Varierar' ? `, median ~${a.wageTable[0].median}` : ''}.` : ''}\n\nSTRIKTA REGLER:\n- Svara BARA baserat på informationen ovan. Hitta ALDRIG på.\n- Svara på svenska, kort. Håll under 200 ord.\n- Avsluta ALLTID med: "Kontakta ${a.parties.unions[0]} för bindande besked."` })),
];
